...
public class SaveStarShip {
    ...
    public boolean isHangarOk(int side1, int side2, int price){
        ...
        if( (float)side1 > (float)side2){
            t1 = (float)side1 / (float)side2 <= 2 ? true : false;
        }else {
            t1 = (float)side2 / (float)side1 <= 2 ? true : false;
        }
        boolean t2 = side1 * side2 >= 1500 ? true : false;
        boolean t3 = (price / side1 / side2) <= 1000 ? true : false;
        boolean T  = t1 && t2 && t3;
        return T;
    }
